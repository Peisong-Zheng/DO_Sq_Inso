...
# ============================================================
BASE = Path('data/')
OUT = Path('results/mcv_coldduration_ccm_lagtest_outputs_v10')
FIG = OUT / 'figures'
TAB = OUT / 'tables'
...
E = 4
TAU = -2
N_SURROGATES = 100               # for row-1 standard CCM only
...
def compute_window_stats(age, cold, warmings, cold_intervals, warm_intervals, window_yr):
    ...
    if win_pts % 2 == 0:
        win_pts += 1
    Eseries, h = moving_sum_centered(warmings.astype(float), win_pts)
    Pseries, _ = moving_mean_centered(cold.astype(float), win_pts)
...
def build_dataset():
    stats = prepare_ch4_sequence(CH4_STATE_FILE)['window_stats'].copy()
...
def ccm_lag_curve_vannes_style(source_series, target_series, E, tau, lag_values,
    ...
    - for each lag, use many bootstrapped/random library sets of fixed length
    ...
    Here we return the median smoothed curve and 5–95% interval across
    bootstrapped library sets.
    """
    rng = np.random.default_rng(rng_seed)
    valid_idx, manifold = build_embedding(source_series, E, tau)
    ...
    # Gaussian smoothing along lag axis for each bootstrap library-set curve
    smooth_curves = gaussian_filter1d(curves, sigma=gauss_sigma, axis=1, mode='nearest')

    curve_df = pd.DataFrame({
        'lag_kyr': lag_values.astype(float),
        'rho_median_smooth': np.nanmedian(smooth_curves, axis=0),
    ...
    best_idx = int(np.nanargmax(curve_df['rho_median_smooth'].to_numpy()))
    summary = {
        'best_lag_kyr': int(lag_values[best_idx]),
        'best_rho': float(curve_df['rho_median_smooth'].iloc[best_idx]),
    }
    return curve_df, summary

def main():
    stats = build_dataset()
    stats.to_csv(TAB / 'aligned_coldduration_orbitals_for_ccm_lagtest.csv', index=False)
    ...
    axes[0].plot(stats['age_yrBP'] / 1000, stats['mean_cold_duration_kyr_raw'], color='0.6', lw=1, label='raw')
    axes[0].plot(stats['age_yrBP'] / 1000, stats['mean_cold_duration_kyr'], color='black', lw=1.2, label='used in CCM')
    axes[0].set_ylabel('cold dur.\\n(kyr)')
    axes[0].legend(frameon=True, fontsize=8)
    for ax, col, lab, cc in zip(
        axes[1:], ['pre', 'obl', 'ecc', 'warming_count_20k'],
        ['pre', 'obl', 'ecc', 'warmings/20k'],
        ['tab:blue', 'tab:green', 'tab:red', 'tab:purple']
    ):
        ax.plot(stats['age_yrBP'] / 1000, stats[col], color=cc, lw=1)
        ax.set_ylabel(lab)
    axes[0].set_title(
        f'Cold-duration inputs (Δt={DOWNSAMPLE_YR} yr, E={E}, tau={TAU}, '
        f'row1 null={N_SURROGATES}, lag bootlibs={LAG_BOOTSTRAP_LIBS}, '
        f'lib={LAG_LIBRARY_SIZE}, smooth_pts={COLD_DURATION_SMOOTH_POINTS})'
    )
    axes[-1].set_xlabel('Age (kyr BP)')
    axes[-1].invert_xaxis()
    fig.tight_layout()
    fig.savefig(FIG / 'fig01_inputs_cold_duration_and_orbitals.png', bbox_inches='tight')
    plt.close(fig)
    ...
    for j, (driver, label) in enumerate(drivers):
        ...
        lag_f, sum_f = ccm_lag_curve_vannes_style(
            ...
            LAG_LIBRARY_SIZE, LAG_BOOTSTRAP_LIBS, LAG_GAUSS_SIGMA, RNG_SEED + j * 100 + 11
        )
        lag_r, sum_r = ccm_lag_curve_vannes_style(
            dseries, metric_series, E, TAU, LAG_RANGE_KYR,
            LAG_LIBRARY_SIZE, LAG_BOOTSTRAP_LIBS, LAG_GAUSS_SIGMA, RNG_SEED + j * 100 + 22
        )
    ...
    res.to_csv(TAB / 'coldduration_ccm_lagtest_results.csv', index=False)

    # Main 2x3 figure
    fig, axes = plt.subplots(2, 3, figsize=(15, 9.0), dpi=140)
    for j, (driver, label) in enumerate(drivers):
        rr = res.loc[res.driver == driver].iloc[0]

        # Row 1: standard CCM
        ax = axes[0, j]
        df_f = std_curves[(driver, 'fwd')]
        df_r = std_curves[(driver, 'rev')]
        ...
        # Row 2: robust lag curves
        ax = axes[1, j]
        df_f = lag_curves[(driver, 'fwd')]
        df_r = lag_curves[(driver, 'rev')]
        ...
        ax.set_title(
            f'robust lag-test: best_fwd={int(rr.best_lag_metric_xmap_orbital_kyr)} kyr, '
            f'best_rev={int(rr.best_lag_orbital_xmap_metric_kyr)} kyr'
        )
        if j == 0:
    ...
    fig.suptitle(
        'Cold-duration focused CCM and lag-test (bidirectional)\\n'
        'Lag-test uses Van Nes-style bootstrapped library sets and Gaussian-smoothed rho-lag curves',
        y=1.02
    )
    fig.subplots_adjust(left=0.06, right=0.98, bottom=0.08, top=0.88, wspace=0.22, hspace=0.28)
    fig.savefig(FIG / 'fig02_coldduration_ccm_and_lagtest_2x3.png')
    plt.close(fig)

    with open(OUT / 'summary.txt', 'w', encoding='utf-8') as f:
        f.write('Cold-duration focused CCM and robust lag-test\\n')
        ...
        f.write(
            ...
            f'(n={LAG_BOOTSTRAP_LIBS}, library size={LAG_LIBRARY_SIZE}) '
            f'+ Gaussian smoothing (sigma={LAG_GAUSS_SIGMA}).\\n\\n'
        )
