...
OUT = SCRIPT_DIR / 'results' / 'mcv_coldduration_ccm_predeamp_lr04_only_v14'
FIG = OUT / 'figures'
TAB = OUT / 'tables'
...
def build_driver_library():
    ...
    return {'pre_raw': pre_df, 'ecc_raw': ecc_df, 'pre_phase': pre_phase_df, 'lr04': lr04_df}
...
def build_dataset_for_proxy(proxy_name, driver_lib):
    ...
    stats = stats.dropna(subset=['mean_cold_duration_kyr', 'pre_phase_like', 'lr04_10k']).copy()
...
def plot_driver_inputs(driver_lib):
    ...
    fig, axes = plt.subplots(2, 1, figsize=(12.5, 6.8), dpi=180)
    ...
    axes[1].invert_xaxis()
    fig.tight_layout()
    fig.savefig(FIG / 'fig02_driver_inputs_predeamp_lr04.png', bbox_inches='tight')
    plt.close(fig)
...
def plot_ccm_comparison(results_by_proxy, std_curves_by_proxy, lag_curves_by_proxy):
    ...
    for p_row, proxy in enumerate(['ch4', 'monsoon']):
        ...
        for j, (driver, label) in enumerate(DRIVER_META):
            ...
            ax.grid(alpha=0.2, ls='--')
    fig.suptitle('Cold-duration CCM and lag-test with de-amplitude PRE + smoothed LR04 (bidirectional)', y=0.995)
    fig.subplots_adjust(left=0.06, right=0.99, bottom=0.05, top=0.92, wspace=0.24, hspace=0.34)
    fig.savefig(FIG / 'fig03_coldduration_ccm_lagtest_predeamp_lr04_only_4x2.png')
    plt.close(fig)
...
def plot_summary(results_by_proxy):
    ...
    fig.tight_layout()
    fig.savefig(FIG / 'fig04_proxy_driver_summary_predeamp_lr04_only.png', bbox_inches='tight')
    plt.close(fig)
...
def main():
    ...
    all_res.to_csv(TAB / 'coldduration_ccm_lagtest_results_all_proxies_predeamp_lr04_only.csv', index=False)
    proxy_summary_df = pd.concat(proxy_summaries, ignore_index=True)
    proxy_summary_df.to_csv(TAB / 'proxy_series_summary_predeamp_lr04_only.csv', index=False)
    ...
    with open(OUT / 'summary.txt', 'w', encoding='utf-8') as f:
        f.write('Cold-duration CCM and robust lag-test with de-amplitude PRE + LR04 10-kyr smooth\n')
