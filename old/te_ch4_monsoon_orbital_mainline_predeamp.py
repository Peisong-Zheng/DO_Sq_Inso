...
- uniform resampling
- high-pass filtering
- alignment with orbital forcing (pre phase-like / obl / ecc)
...
Here the "pre" driver is not the raw precession series. It is replaced by
a de-amplitude pre phase-like series derived from pre/ecc, so that the fast
precession-like oscillation is retained while the low-frequency eccentricity
amplitude modulation is largely removed.
...
def load_pre_phase_like_series(pre_path: Path, ecc_path: Path, value_name: str = "pre") -> pd.DataFrame:
    """
    Replace raw precession with a de-amplitude "phase-like" pre series.

    Method kept consistent with the earlier CCM experiment:
    - load raw pre and ecc on their native orbital grids
    - merge by common age
    - use eccentricity as a low-frequency amplitude-envelope proxy
    - divide pre by ecc (with a low-percentile floor for numerical stability)
    - rescale back to roughly the original pre standard deviation

    The returned dataframe keeps the column name ``value_name`` so that the rest
    of the TE workflow can remain unchanged.
    """
    pre_df = load_orbital_series(pre_path, "pre_raw")
    ecc_df = load_orbital_series(ecc_path, "ecc_raw")

    merged = pre_df.merge(ecc_df, on="age", how="inner").sort_values("age").reset_index(drop=True)
    pre = merged["pre_raw"].to_numpy(dtype=float)
    ecc = merged["ecc_raw"].to_numpy(dtype=float)

    ecc_floor = float(np.nanpercentile(ecc, 5))
    ...
    sd_pre = float(np.nanstd(pre))
    sd_phase = float(np.nanstd(pre_phase_like))
    if sd_phase > 0 and np.isfinite(sd_phase):
        pre_phase_like = pre_phase_like * (sd_pre / sd_phase)

    out = pd.DataFrame({"age": merged["age"].to_numpy(dtype=float), value_name: pre_phase_like})
    return out
...
def plot_aligned_series(df_aligned: pd.DataFrame, target_col: str, target_label: str, out_path: Path) -> None:
    ...
    labels = [target_label, "Pre phase-like", "Obliquity", "Eccentricity"]
...
def main() -> None:
    ...
    orbital = {
        "pre": load_pre_phase_like_series(cfg.data_dir / cfg.pre_file, cfg.data_dir / cfg.ecc_file, "pre"),
    ...
    metadata = {
        ...
        "pre_driver_note": "pre column replaced by de-amplitude pre phase-like series derived from pre/ecc",
    }
