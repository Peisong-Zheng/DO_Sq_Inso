...
#    NGRIP 末次冰期至 MIS 5 期间的 Greenland Stadial (GS) 区间
# ============================================================
GS_START = np.array([
...
# ============================================================
# 2. Detector settings
#    这里不再硬编码“selected parameters”，而是对每个代理单独网格搜索
# ============================================================
DT_INPUT = 10          # 10 yr interpolation backbone for filtering
...
# 若 age 最大值明显小于几千，则很可能是 kyr BP；自动转成 yr BP
AGE_UNIT_KYR_THRESHOLD = 5_000

PROXIES = [
    {
        'name': 'ch4',
        'display': 'CH$_4$',
        'filename': 'CH4_AICC2023.xlsx',
        'value_column': 'ch4',
        'full_output_prefix': 'ch4_mcv_square_wave_selected_full_50yr',
        'cal_output_prefix': 'ch4_mcv_square_wave_selected_calibration_50yr',
        'line_label': 'Filtered CH$_4$',
    },
    {
        'name': 'monsoon',
        'display': r'Monsoon $\delta^{18}$O',
        'filename': 'monsoon.xlsx',
        'value_column': 'd18O',
        'full_output_prefix': 'monsoon_mcv_square_wave_selected_full_50yr',
        'cal_output_prefix': 'monsoon_mcv_square_wave_selected_calibration_50yr',
        'line_label': r'Filtered monsoon $\delta^{18}$O',
    },
]
...
def resolve_input_file(filename: str) -> Path:
    ...
    for path in candidates:
        if path.exists():
            return path
    raise FileNotFoundError(f'Cannot find {filename}. Expected in either {BASE} or {SCRIPT_DIR}.')



def load_proxy_table(filename: str, value_column: str) -> pd.DataFrame:
    """读取代理数据，并统一为两列：age, value。

    兼容 age 单位是 yr BP 或 kyr BP 的情况：
    - 如果最大 age < 5000，则视为 kyr BP，自动 ×1000 转为 yr BP。
    """
    xlsx = resolve_input_file(filename)
    raw = pd.read_excel(xlsx)

    required = {'age', value_column}
    if not required.issubset(raw.columns):
        raise ValueError(f'{filename} must contain columns {required}; got {list(raw.columns)}')

    raw = raw[['age', value_column]].dropna().sort_values('age').reset_index(drop=True)
    raw.columns = ['age', 'value']

    if raw['age'].max() < AGE_UNIT_KYR_THRESHOLD:
        raw['age'] = raw['age'] * 1000.0

    return raw



def build_filtered_series(raw: pd.DataFrame) -> dict[str, np.ndarray | pd.DataFrame]:
    """插值到 10 yr，做高通滤波，再重采样到 50 yr 用于 MCV detector。"""
    raw_use = raw[raw['age'] >= 1000].copy()

    ages_10 = np.arange(1000, int(np.floor(raw_use['age'].max())) + 1, DT_INPUT)
    value_10 = np.interp(ages_10, raw_use['age'].to_numpy(), raw_use['value'].to_numpy())

    fs = 1.0 / DT_INPUT
    fc = 1.0 / HP_CUTOFF
    wn = fc / (fs * 0.5)
    b, a = butter(4, wn, btype='highpass')
    value_hp_10 = filtfilt(b, a, value_10)

    ages_50 = np.arange(int(ages_10.min()), int(ages_10.max()) + 1, DT_DETECT)
    value_hp_50 = np.interp(ages_50, ages_10, value_hp_10)

    return {
        'raw_use': raw_use,
        'ages_10': ages_10,
        'value_10': value_10,
        'value_hp_10': value_hp_10,
        'ages_50': ages_50,
        'value_hp_50': value_hp_50,
    }
...
def save_square_wave_to_data(ages, values, states, segment_id, flips, out_prefix: str):
    ...
    out_path = BASE / f'{out_prefix}.csv'
    out.to_csv(out_path, index=False)
    pd.DataFrame({'transition_age_yrBP': np.asarray(flips, int)}).to_csv(
        BASE / f'{out_prefix}_transitions.csv', index=False
    )
    return out_path



def compute_grid(cal_age: np.ndarray, cal_resid: np.ndarray, cal_median: float) -> tuple[pd.DataFrame, pd.DataFrame]:
    rows = []
    for amp_sigma in AMP_GRID:
        for min_persistence in PERSIST_GRID:
            flips, segs, states, segment_id = apply_detector(
                ...
                min_persistence_yr=float(min_persistence),
                dt_yr=DT_DETECT,
            )
            for tol in TOLS:
...
def select_best_params(grid_wide: pd.DataFrame) -> pd.Series:
    """按主评分选最佳参数，再用 F1 / precision / recall 打破平手。"""
    sub = grid_wide.sort_values(
        ['score_primary', 'f1_300', 'precision_300', 'recall_300', 'f1_200', 'f1_100'],
...
def summarize_proxy(proxy: dict) -> dict:
    raw = load_proxy_table(proxy['filename'], proxy['value_column'])
    series = build_filtered_series(raw)

    # calibration window
    cal_age, cal_resid = crop_to_ref_window(series['ages_50'], series['value_hp_50'])
    ...
    sel_flips, sel_segs, sel_states, sel_segment_id = apply_detector(
        cal_age, cal_resid, cal_median,
        amp_sigma=best_amp,
        min_persistence_yr=best_persist,
        dt_yr=DT_DETECT,
    )
    ...
    full_resid_50 = series['value_hp_50']
    ...
    save_square_wave_to_data(
        ...
        flips=full_flips,
        out_prefix=proxy['full_output_prefix'],
    )
    save_square_wave_to_data(
        ages=cal_age,
        values=cal_resid,
        ...
        flips=sel_flips,
        out_prefix=proxy['cal_output_prefix'],
    )
    ...
    return {
        ...
        'best': best,
        'best_amp': best_amp,
        'best_persist': best_persist,
        'sel_flips': sel_flips,
        'sel_segs': sel_segs,
        'sel_states': sel_states,
        'sel_segment_id': sel_segment_id,
...
def plot_heatmaps_combined(results_by_proxy: dict[str, dict]):
    fig, axes = plt.subplots(1, 2, figsize=(13.0, 5.6), constrained_layout=True)
    for ax, key in zip(axes, ['ch4', 'monsoon']):
        res = results_by_proxy[key]
        sub = res['grid_wide'].copy()
        ...
        x = list(piv.columns).index(res['best_amp'])
        y = list(piv.index).index(res['best_persist'])
        ax.scatter([x], [y], s=90, facecolor='none', edgecolor='white', linewidth=1.8, zorder=3)
        ax.scatter([x], [y], s=22, color='white', zorder=3)
        ax.set_title(
            f"{res['proxy']['display']} calibration grid\n"
            f"best: amp={res['best_amp']:.2f}, persist={int(res['best_persist'])} yr"
        )
        cb = fig.colorbar(im, ax=ax, pad=0.02)
        cb.set_label('F1 score at ±300 yr')

    fig.savefig(FIG_DIR / 'fig01_calibration_heatmaps_ch4_monsoon.png', dpi=240)
    plt.close(fig)



def plot_sequence_overview_generic(res: dict, out_name: str):
    cal_age = res['cal_age']
    cal_resid = res['cal_resid']
    cal_med = res['cal_median']
    det_states = res['sel_states']
    det_trans = res['sel_flips']
    match_df = res['match_df']
    unmatched_ref = res['unmatched_ref']
    unmatched_det = res['unmatched_det']
    ...
    ax0.set_ylabel(res['proxy']['line_label'])
    ax0.set_title(
        f"{res['proxy']['display']} calibration-window MCV detection | "
        f"best amp={res['best_amp']:.2f}, persist={int(res['best_persist'])} yr"
    )

    ax1 = fig.add_subplot(gs[1, 0], sharex=ax0)
    ref_state_on_cal = REF_STATE[np.searchsorted(REF_AGES, cal_age)]
    ax1.step(cal_age / 1000, ref_state_on_cal, where='post', color='tab:blue', linewidth=1.6)
    ...
    if not unmatched_det.empty:
        ax3.scatter(unmatched_det['det_age_yrBP'] / 1000, np.full(len(unmatched_det), 0.20),
                    s=18, color='tab:red', label='extra detection')
    ax3.text(12.2, 0.86, 'NGRIP transitions', color='tab:blue', fontsize=9)
    ax3.text(12.2, 0.06, f"{res['proxy']['display']} transitions", color='tab:red', fontsize=9)
...
def plot_pairing_summary_generic(res: dict, out_name: str):
    cal_age = res['cal_age']
    det_trans = res['sel_flips']
    match_df = res['match_df']
    base_metrics = res['base_metrics']
    sel_metrics = res['sel_metrics']
    ...
    ax.set_ylabel(f"{res['proxy']['display']} detected transition age (kyr BP)")
    ax.set_title('Matched events relative to the 1:1 line')
    ax.set_xlim(12, 119)
    ax.set_ylim(12, 119)

    ax = fig.add_subplot(gs[0, 1])
    cum_ref = cumulative_count_curve(cal_age, REF_TRANSITIONS)
    ...
    ax.step(cal_age / 1000, cum_det, where='post', linewidth=1.8, label=f"{res['proxy']['display']} detector")
    ...
    ax.set_ylabel('Score')
    ax.set_title(
        f"Selection improves agreement | best amp={res['best_amp']:.2f}, persist={int(res['best_persist'])} yr"
    )
...
def plot_windowed_comparison_generic(res: dict, out_name: str):
    det_trans = res['sel_flips']
    match_df = res['match_df']
    ...
    for ax, (x0, x1) in zip(axes, windows):
        ...
        ax.text(x0 + 0.2, 0.06, res['proxy']['display'], color='tab:red', fontsize=9)
        ax.set_xlim(x0, x1)
        ax.set_ylim(0, 1)
        ax.set_yticks([])
    ...
    fig.suptitle(f"Window-by-window event comparison for {res['proxy']['display']}", y=0.995)
    fig.tight_layout()
    fig.savefig(FIG_DIR / out_name, dpi=240)
    plt.close(fig)
...
def plot_full_records_combined(results_by_proxy: dict[str, dict]):
    ch4 = results_by_proxy['ch4']
    monsoon = results_by_proxy['monsoon']

    fig = plt.figure(figsize=(13.0, 11.4))
    gs = fig.add_gridspec(4, 1, height_ratios=[2.2, 0.9, 2.2, 0.9], hspace=0.12)

    ax0 = fig.add_subplot(gs[0, 0])
    ax0.plot(ch4['series']['ages_10'] / 1000, ch4['series']['value_hp_10'], color='0.2', linewidth=0.8)
    ax0.axvspan(AGE_MIN / 1000, AGE_MAX / 1000, color='0.92', zorder=0)
    for t in ch4['full_flips']:
        ax0.axvline(t / 1000, color='tab:red', alpha=0.10, linewidth=0.6)
    ax0.set_ylabel(ch4['proxy']['line_label'])
    ax0.set_title(
        f"CH$_4$ full record | best amp={ch4['best_amp']:.2f}, persist={int(ch4['best_persist'])} yr"
    )
    ax0.set_xlim(ch4['series']['ages_10'].min() / 1000, ch4['series']['ages_10'].max() / 1000)

    ax1 = fig.add_subplot(gs[1, 0], sharex=ax0)
    ax1.step(ch4['full_age_50'] / 1000, ch4['full_states'], where='post', color='tab:red', linewidth=1.5)
    ax1.axvspan(AGE_MIN / 1000, AGE_MAX / 1000, color='0.92', zorder=0)
    ax1.set_ylim(-0.2, 1.2)
    ax1.set_yticks([0, 1])
    ax1.set_yticklabels(['low', 'high'])
    ax1.set_ylabel('CH$_4$ MCV')

    ax2 = fig.add_subplot(gs[2, 0], sharex=ax0)
    ax2.plot(monsoon['series']['ages_10'] / 1000, monsoon['series']['value_hp_10'], color='0.2', linewidth=0.8)
    ax2.axvspan(AGE_MIN / 1000, AGE_MAX / 1000, color='0.92', zorder=0)
    for t in monsoon['full_flips']:
        ax2.axvline(t / 1000, color='tab:purple', alpha=0.10, linewidth=0.6)
    ax2.set_ylabel(monsoon['proxy']['line_label'])
    ax2.set_title(
        fr"Monsoon $\delta^{{18}}$O full record | best amp={monsoon['best_amp']:.2f}, persist={int(monsoon['best_persist'])} yr"
    )

    ax3 = fig.add_subplot(gs[3, 0], sharex=ax0)
    ax3.step(monsoon['full_age_50'] / 1000, monsoon['full_states'], where='post', color='tab:purple', linewidth=1.5)
    ax3.axvspan(AGE_MIN / 1000, AGE_MAX / 1000, color='0.92', zorder=0)
    ax3.set_ylim(-0.2, 1.2)
    ax3.set_yticks([0, 1])
    ax3.set_yticklabels(['low', 'high'])
    ax3.set_ylabel('Monsoon MCV')
    ax3.set_xlabel('Age (kyr BP)')
    ...
    fig.tight_layout()
    fig.savefig(FIG_DIR / 'fig08_full_records_square_wave.png', dpi=240)
    plt.close(fig)
...
def main():
    ...
    for proxy in PROXIES:
        ...
        best_by_tol_rows = []
        for tol in TOLS:
            sub = res['grid'][res['grid']['tol_yr'] == tol].sort_values(
                ['f1', 'precision', 'recall'], ascending=[False, False, False]
            )
            best_by_tol_rows.append(sub.iloc[0])
        save_table(pd.DataFrame(best_by_tol_rows), f"best_by_tolerance_{proxy['name']}.csv")
        ...
        seg_rows = []
        for k, seg in enumerate(res['sel_segs']):
            seg_rows.append({
                'segment_idx': k,
                'state': seg['state'],
                'start_age_yrBP': res['cal_age'][seg['start']],
                'end_age_yrBP': res['cal_age'][seg['end'] - 1],
                'duration_yr': seg['dur_yr'],
                'max_abs_amp': seg['amp'],
            })
        save_table(pd.DataFrame(seg_rows), f"selected_segments_{proxy['name']}.csv")

        metric_rows = []
        for det_name, metrics_by_tol, flips in [
            ('baseline_crossing', res['base_metrics'], res['base_flips']),
            (f"selected_{res['best_amp']:.2f}_{int(res['best_persist'])}yr", res['sel_metrics'], res['sel_flips']),
        ]:
            for tol in TOLS:
                metric_rows.append({
                    'detector': det_name,
        ...
        full_summary = pd.DataFrame([
            {
                'proxy': proxy['name'],
                'amp_sigma': res['best_amp'],
                'min_persistence_yr': res['best_persist'],
                'age_min_yrBP': int(res['full_age_50'].min()),
                'age_max_yrBP': int(res['full_age_50'].max()),
                'n_transitions_calibration': len(res['sel_flips']),
                'n_segments_calibration': len(res['sel_segs']),
                'n_matched_tol300': res['sel_metrics'][300]['n_match'],
                'precision_tol300': res['sel_metrics'][300]['precision'],
                'recall_tol300': res['sel_metrics'][300]['recall'],
                'f1_tol300': res['sel_metrics'][300]['f1'],
                'median_abs_age_error_tol300_yr': res['sel_metrics'][300]['median_abs_age_error_yr'],
                'mean_abs_age_error_tol300_yr': res['sel_metrics'][300]['mean_abs_age_error_yr'],
                'n_transitions_full': len(res['full_flips']),
                'n_segments_full': len(res['full_segs']),
            }
        ])
    ...
    # 图：先做联合热图，再分别做单代理诊断图，最后做联合 full-record 图
    plot_heatmaps_combined(results_by_proxy)

    plot_sequence_overview_generic(results_by_proxy['ch4'], 'fig02_ch4_sequence_overview.png')
    plot_pairing_summary_generic(results_by_proxy['ch4'], 'fig03_ch4_pairing_summary.png')
    plot_windowed_comparison_generic(results_by_proxy['ch4'], 'fig04_ch4_windowed_event_comparison.png')

    plot_sequence_overview_generic(results_by_proxy['monsoon'], 'fig05_monsoon_sequence_overview.png')
    plot_pairing_summary_generic(results_by_proxy['monsoon'], 'fig06_monsoon_pairing_summary.png')
    plot_windowed_comparison_generic(results_by_proxy['monsoon'], 'fig07_monsoon_windowed_event_comparison.png')

    plot_full_records_combined(results_by_proxy)

    print('Done.')
    for key in ['ch4', 'monsoon']:
        res = results_by_proxy[key]
        print(
            f"{key}: best amp_sigma={res['best_amp']:.2f}, "
            f"min_persistence={int(res['best_persist'])} yr, "
            f"matched at ±300 yr = {res['sel_metrics'][300]['n_match']} / {len(REF_TRANSITIONS)}, "
            f"full transitions = {len(res['full_flips'])}"
        )
