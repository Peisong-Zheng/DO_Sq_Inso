...
OUT = SCRIPT_DIR / 'results' / 'mcv_coldduration_ccm_lagtest_outputs_ch4_monsoon_v11'
FIG = OUT / 'figures'
TAB = OUT / 'tables'
...
OBL_FILE = BASE / 'obl_800_inter100.txt'
...
# Cold-duration smoothing before CCM
COLD_DURATION_SMOOTH_POINTS = 0   # 0 = none, else centered rolling mean points

# Row-2 robust lag-test settings (Van Nes style)
LAG_LIBRARY_SIZE = 300            # requested library length; script will adapt downward if needed
LAG_BOOTSTRAP_LIBS = 100
LAG_GAUSS_SIGMA = 1.0
LAG_LIBRARY_FRACTION_CAP = 0.60   # when sequences are short, use at most this fraction of the minimum lag-valid rows
...
def centered_smooth_with_edge_handling(x, win_pts):
    ...
    return (
        pd.Series(x)
        .rolling(window=win_pts, center=True, min_periods=1)
        .mean()
        .to_numpy()
    )
...
def interp_to_grid(age, src_age, src_val):
    ...
    return np.interp(
        age,
        np.asarray(src_age, float)[order],
        np.asarray(src_val, float)[order]
    )
...
def build_dataset_for_proxy(proxy_name):
    ...
    stats['mean_cold_duration_kyr'] = centered_smooth_with_edge_handling(
        stats['mean_cold_duration_kyr_raw'].to_numpy(),
        smooth_pts
    )

    # Monsoon series can have a few NaNs where the window contains no full cold interval.
    # Drop those rows so CCM/lag tests are evaluated only where the metric is defined.
...
# Row-1: standard CCM with permutation null

def libsizes_for_n(n_valid):
    vals = np.unique(
        np.round(np.linspace(max(E + 5, int(0.1 * n_valid)), int(0.9 * n_valid), LIB_POINTS)).astype(int)
    )
...
def ccm_curve_and_nulls_fast(source_series, target_series, E, tau, n_surrogates, rng_seed):
    ...
    subsets = {
        int(L): np.sort(rng.choice(valid_idx, size=int(L), replace=False))
        for L in libs
    }
    ...
    p = float((np.sum(null_curves[:, -1] >= obs[-1]) + 1) / (n_surrogates + 1))
    del distmat, null_curves
    gc.collect()
    return curve_df, float(obs[-1]), p


# Row-2: robust lag curves, Van Nes style
...
def choose_effective_lag_library_size(n_valid, lag_values, E, tau, requested, frac_cap):
    ...
    min_rows = min(
        np.sum((valid_idx + lag >= 0) & (valid_idx + lag < n_valid))
...
def ccm_lag_curve_vannes_style(source_series, target_series, E, tau, lag_values,
                               library_size, n_boot, gauss_sigma, rng_seed):
    """
    Van Nes et al. 2015 style:
    - for each lag, use many random library sets of fixed length
    - evaluate CCM skill rho for each library set
    - smooth rho-versus-lag relation with a Gaussian filter
    - determine optimal lag from the smoothed relation

    Returns the median smoothed curve and 5–95% interval across
    bootstrapped/random library-set curves.
    """
    rng = np.random.default_rng(rng_seed)
    valid_idx, manifold = build_embedding(source_series, E, tau)
    lag_values = np.asarray(lag_values, int)
    ...
    if np.all(~np.isfinite(med)):
        ...
        summary = {
            'best_lag_kyr': int(lag_values[best_idx]),
            'best_rho': float(med[best_idx]),
        }
...
def plot_inputs_comparison(dataset_by_proxy):
    ...
    for ax, lab in zip(axes[1:], ['pre', 'obl', 'ecc', 'warmings/20k']):
        ax.set_ylabel(lab)
    axes[0].set_title(
        f'Cold-duration inputs for CH$_4$ and monsoon (Δt={DOWNSAMPLE_YR} yr, E={E}, tau={TAU}, '
    ...
    axes[-1].invert_xaxis()
    fig.tight_layout()
    fig.savefig(FIG / 'fig01_inputs_cold_duration_and_orbitals_comparison.png', bbox_inches='tight')
    plt.close(fig)
...
def plot_ccm_comparison(results_by_proxy, std_curves_by_proxy, lag_curves_by_proxy):
    ...
    for p_row, proxy in enumerate(['ch4', 'monsoon']):
        ...
        for j, (driver, label) in enumerate(drivers):
            ...
            ax.set_title(
                f'{PROXY_LABELS[proxy]} · {label}\n'
                f'CCM: p_fwd={rr.p_metric_xmap_orbital:.3f}, p_rev={rr.p_orbital_xmap_metric:.3f}'
            )
            if j == 0:
            ...
            ax.set_title(
                f'robust lag-test: best_fwd={rr.best_lag_metric_xmap_orbital_kyr:.0f} kyr, '
                f'best_rev={rr.best_lag_orbital_xmap_metric_kyr:.0f} kyr\n'
                f'effective lib={int(rr.lag_library_effective)}'
            )
            if j == 0:
    ...
    fig.suptitle(
        'Cold-duration focused CCM and lag-test for CH$_4$ vs monsoon d$^{18}$O (bidirectional)\n'
        'Lag-test uses Van Nes-style bootstrapped library sets and Gaussian-smoothed rho-lag curves',
        y=0.995
    )
    fig.subplots_adjust(left=0.06, right=0.98, bottom=0.05, top=0.93, wspace=0.22, hspace=0.34)
    fig.savefig(FIG / 'fig02_coldduration_ccm_and_lagtest_comparison_4x3.png')
    plt.close(fig)
...
def plot_proxy_driver_contrast(results_by_proxy):
    ...
    for ax, (col, ylabel) in zip(axes.flat, metrics):
        ...
        ax.bar(x - width/2, ch4, width, label='CH$_4$', color=PROXY_COLORS['ch4'], alpha=0.85)
        ax.bar(x + width/2, mon, width, label='Monsoon d$^{18}$O', color=PROXY_COLORS['monsoon'], alpha=0.85)
        ax.set_xticks(x)
        ax.set_xticklabels(labels)
        ax.set_ylabel(ylabel)
        ax.grid(axis='y', alpha=0.2, ls='--')
    axes[0, 0].legend(frameon=True, fontsize=8)
    fig.suptitle('CH$_4$ versus monsoon: summary comparison across orbital drivers', y=0.98)
    fig.tight_layout()
    fig.savefig(FIG / 'fig03_proxy_driver_summary_comparison.png')
    plt.close(fig)
...
def run_proxy(proxy, proxy_seed_offset):
    stats = build_dataset_for_proxy(proxy)
    ...
    for j, (driver, label) in enumerate(drivers):
        ...
        # Row 1: standard CCM
        f_curve, f_obs, f_p = ccm_curve_and_nulls_fast(
            metric_series, dseries, E, TAU, N_SURROGATES, RNG_SEED + proxy_seed_offset + j * 10 + 1
        )
        r_curve, r_obs, r_p = ccm_curve_and_nulls_fast(
            dseries, metric_series, E, TAU, N_SURROGATES, RNG_SEED + proxy_seed_offset + j * 10 + 2
        )

        # Row 2: Van Nes style robust lag curves
        lag_f, sum_f = ccm_lag_curve_vannes_style(
            metric_series, dseries, E, TAU, LAG_RANGE_KYR,
            lib_info['effective_library_size'], LAG_BOOTSTRAP_LIBS, LAG_GAUSS_SIGMA,
            RNG_SEED + proxy_seed_offset + j * 100 + 11
        )
        lag_r, sum_r = ccm_lag_curve_vannes_style(
            dseries, metric_series, E, TAU, LAG_RANGE_KYR,
            lib_info['effective_library_size'], LAG_BOOTSTRAP_LIBS, LAG_GAUSS_SIGMA,
            RNG_SEED + proxy_seed_offset + j * 100 + 22
        )
...
def main():
    ...
    all_results = {
        'ch4': run_proxy('ch4', 0),
        'monsoon': run_proxy('monsoon', 1000),
    }
    ...
    for proxy, bundle in all_results.items():
        ...
        comparison_rows.append({
            'proxy': proxy,
            'proxy_label': PROXY_LABELS[proxy],
    ...
    with open(OUT / 'summary.txt', 'w', encoding='utf-8') as f:
        ...
        f.write(
            f'Lag-test method: Van Nes style bootstrapped library sets '
            f'(n={LAG_BOOTSTRAP_LIBS}, requested library size={LAG_LIBRARY_SIZE}, '
            f'fraction cap={LAG_LIBRARY_FRACTION_CAP}) + Gaussian smoothing '
            f'(sigma={LAG_GAUSS_SIGMA}).\n\n'
        )
        f.write('Lag sign convention: arrays are sorted from old to young age, so +lag means the target series is sampled at younger ages than the source; -lag means older ages.\n\n')
        for proxy, bundle in all_results.items():
            f.write(f'Proxy: {proxy} ({PROXY_LABELS[proxy]})\n')
